    ...
    defaultPackage = let
      ...
      pyprojectOverrides =
        pkgs.lib.composeExtensions (final: prev: {
          torch-sparse = prev.torch-sparse.overrideAttrs {
            nativeBuildInputs =
              prev.torch-sparse.nativeBuildInputs
              ++ [
                pkgs.which
              ];
          };
          torch-scatter = prev.torch-scatter.overrideAttrs {
            nativeBuildInputs =
              prev.torch-scatter.nativeBuildInputs
              ++ [
                pkgs.which
              ];
          };
          docrep = prev.docrep.overrideAttrs {
            nativeBuildInputs =
              prev.docrep.nativeBuildInputs
              or []
              ++ [
                (final.resolveBuildSystem {wheel = [];})
              ];
          };
          session-info = prev.session-info.overrideAttrs {
            nativeBuildInputs =
              prev.docrep.nativeBuildInputs
              or []
              ++ [
